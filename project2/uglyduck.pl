...
%gameLoopComputerHuman() %this is the loop of the computer vs player mode, similar to the player vs computer but now the computer goes first
gameLoopComputerHuman :-
    ...
    incrementMove(Move,NewMove),nl,nl,
    retract(level(Level)),
    computerMove(Finalboard-Length,1,Level,ComputerBoard),
