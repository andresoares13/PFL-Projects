import Project1 (normPol,sumPol,mulPol,derivePoly,uiNormPol,uiSumPol,uiDerivePol,uiMulPol,programUI,testNormPol,testSumPol,testDerivePol,testMulPol,polyParse, prop_sumMon, prop_mulMon ,prop_deriveMon, prop_normMon)
import Test.QuickCheck


--Test File, here you can run the funciton "tests" and you will have an user interface where you can either input your own polynomials or see our predefined tests.
--Please note that in this file we use our parsing function in our predefined tests so if you would like to run tests by inputing directly in the format we created, please run the tests we have commented bellow in this file
--We implemented some quickCheck tests that can be run with the call "quickCheck (function_name)", those calls are also written in comments bellow
    ...
    if (option == (show 1)) then
        ...
        if (option == (show 2)) then
            do 
              ...
              putStr ("4. Polynomials -> (7xy - 6y^2 + 4x + 5)  +  (-7yx + 6y^2 - 4x - 5)\n\n" ++ "Result:   " ++ testSumPol (polyParse "7xy -6y^2 +4x+5") (polyParse "-7yx + 6y^2 - 4x - 5") ++ "\n\nExpected:   0\n\n")
              putStr ("5. Polynomials -> (10x^2 - 10y^2 - x - 10x^2 + 5)  +  (5z^6 + 10y^2 - 19 + 4x^19 - 7x^4)\n\n" ++ "Result:   " ++ testSumPol (polyParse "10x^2 - 10y^2 - x - 10x^2 + 5") (polyParse "5z^6 + 10y^2 - 19 + 4x^19 - 7x^4") ++ "\n\nExpected:   4x^19 + 5z^6 - 7x^4 - x - 14\n\n")
              ...
              putStr ("2. Polynomials -> (0)  *  (3y^3 + 4zx)\n\n" ++ "Result:   " ++ testMulPol (polyParse "0") (polyParse "3y^3 + 4zx") ++ "\n\nExpected:   0\n\n")
...
--Tests Section -- test list in our format
...
--QuickCheck tests Sectionm -> this are calls to use quickCheck

--quickCheck prop_sumMon //passes all tests

--quickCheck prop_mulMon //passes all tests

--quickCheck prop_deriveMon  //note that in this test, due to the restrictions necessary to test it, a very high number of tests are discarded by quickCheck but it still passes some tests, between 36 and 52

--quickCheck prop_normMon   //note that in this test, there were too many restrictions to make it somewhat of a valid poly formart and therefore it discards pretty much all tests and only passes (at most from running it a couple of times) between 0 and 4